...
const IDENTITY = {
	generatorId: 'odd_one_out.numbers.v2',
	version: 2,
} as const
...
function kindsForDifficulty(difficulty: Difficulty): PropertyKind[] {
	...
	if (difficulty === 4) {
		return ['divisible_by', 'perfect_square', 'multiples']
	}
	return ['perfect_square', 'prime', 'multiples']
}
...
function buildMultiples(rng: RandomSource): {
	numbers: number[]
	oddIndex: number
	hint1: string
	hint2: string
	...
	const base = rng.pick([3, 4, 5, 6])
	const majority = [base * 2, base * 3, base * 4]
	let oddOne = rng.nextInt(2, 40)
	let attempts = 0
	while ((oddOne % base === 0 || majority.includes(oddOne)) && attempts < 40) {
		oddOne = rng.nextInt(2, 40)
		attempts += 1
	}
	if (oddOne % base === 0) {
		oddOne = base * 4 + 1
	...
	return {
		numbers,
		oddIndex,
		hint1: 'Три числа кратны одному и тому же числу.',
		hint2: `Три числа делятся на ${base}.`,
		explanation: `${majority.join(', ')} — кратные ${base}. ${oddOne} не кратно ${base}.`,
		mechanic: `multiples_${base}`,
	}
}
...
export const oddOneOutNumbersGeneratorV2: PuzzleGenerator = {
	...IDENTITY,
	displayName: 'Найди лишнее — числа v2',
